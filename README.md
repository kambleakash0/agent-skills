...
| Server | Description |
| ------ | ----------- |
| [ast-editor](./mcp-servers/ast-editor/README.md) | AST-targeted code editing MCP server with **35 surgical tools**: functions, classes, methods, fields, parameters, imports, comments, docstrings, dict/list editing (JSON/YAML/TOML AND Python/JS/TS literals), **in-body snippet editing** (`replace_in_body`, `delete_in_body`, `insert_in_body`), and **AST reader tools** (`read_symbol`, `read_interface`, `read_imports`) for 10-20x input token savings. Dotted targets descend into closures (Go `stdioCmd.RunE`, TS `app.handler`). Built on tree-sitter. Supports **11 languages**: Python, JavaScript, TypeScript, C, C++, Ruby, Go, Java, JSON, YAML, and TOML. |
